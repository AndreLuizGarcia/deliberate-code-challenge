...
describe('Categories page', () => {
  it('should render correctly', () => {
    ...
    myhealthfinderApiMock.get.mockImplementation(() =>
      Promise.resolve({ data: { Result: { Items: { Item: response } } } })
    )

    shallow(<Categories />)

    expect(myhealthfinderApiMock.get).toHaveBeenCalledWith('/itemlist.json')
  })

  it('should return an error from request', () => {
    const setCategoriesMock = jest.fn()
  ...
  it('should show loading message when is loadingd', () => {
    jest
      .spyOn(React, 'useState')
  ...
  it('should render the categories correctly', () => {
    ...
    jest
      .spyOn(React, 'useState')
      .mockImplementationOnce(() => [response, jest.fn()])

    const wrapper = shallow(<Categories />)
    const li = wrapper.find('li')

    expect(li.text()).toBe('Keep Your Heart Healthy')
  })
})
